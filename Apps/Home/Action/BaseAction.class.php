...
class BaseAction extends Controller {
	   ...
	   $config = array(
		...
		if(!$rs){
			if(empty($app)||$app!="yes"){
				...
				file_put_contents("../webhook/test/uploadPic.error", $upload->getError());
				$this->json(200,"上传图片失败");
			}
		}else{

			file_put_contents("../webhook/test/uploadPic.post", json_encode($_POST));
			...
			if(empty($app)||$app!="yes"){
				...
				$imgInfo = array(
						//'imgSavePath'	=>	$rs['Filedata']['savepath'].$rs['Filedata']['savename'],
						//'thumbSavePath'	=>	$rs['Filedata']['savepath'].$rs['Filedata']['savethumbname'],
						'imgUrl'	=> $rs['Filedata']['savepath'].$rs['Filedata']['savename'],
						//'thumbUrl'	=>	C('SERVER').$rs['Filedata']['savepath'].$rs['Filedata']['savethumbname']
					);
				$imgSavePath = $rs['Filedata']['savepath'].$rs['Filedata']['savename'];
				$type = I('type');
				$id = I('id');
				...
				switch ($type) {
					case 'goods':
						$img["goodsImg"] = $rs['Filedata']['savepath'].$rs['Filedata']['savename'];
						M('goods')->where("goodsId=".$id)->save($img);
						break;
					case 'shops':
						$img["shopImg"] = $rs['Filedata']['savepath'].$rs['Filedata']['savename'];
						M('shops')->where("shopId=".$id)->save($img);
						break;
					case 'users':
						$img["userPhoto"] = $rs['Filedata']['savepath'].$rs['Filedata']['savename'];
						M('users')->where("userId=".$id)->save($img);
						break;
					default:
	
						break;
				}
				file_put_contents("../webhook/test/uploadPic.img", json_encode($img));
				$this->json(100,"上传图片成功",$imgInfo,"imgInfo");
			}
   ...
   /**
	...
	public function jsonPage($code, $message = '',$totalPage, $data = array(),$infoType) {
